...
const OrderDecisionService = require('../../backend/src/services/orderDecision.service');
...
module.exports = (bot) => {
    ...
    bot.callbackQuery(/^confirm_(.+)$/, async (ctx) => {
        try {
            ...
            order = await OrderDecisionService.acceptOrder({
                orderId,
                branchId: order.branch,
                actor: ctx.from.first_name || String(ctx.from.id),
                source: 'telegram',
            });
            ...
            await ctx.answerCallbackQuery(error.statusCode ? error.message : 'Xato yuz berdi');
        }
    });
    ...
    bot.callbackQuery(/^reject_reason_(.+)_(nodrug|other)$/, async (ctx) => {
        try {
            ...
            const rejected = await OrderDecisionService.rejectOrder({
                orderId,
                branchId: order.branch,
                reason: reasonText,
                comment: reasonText,
            ...
            // Xabarni yangilash
            await ctx.editMessageText(
                `❌ <b>Rad etildi</b> — ${rejected.customerName}\n` +
                `📦 #${rejected.orderNumber}\n` +
                `💵 ${rejected.total.toLocaleString()} so'm\n` +
                `📝 Sabab: ${reasonText}\n` +
                `🕐 ${new Date().toLocaleTimeString('uz-UZ', { hour: '2-digit', minute: '2-digit' })}`,
                { parse_mode: 'HTML' }
            );
            ...
            await ctx.answerCallbackQuery(error.statusCode ? error.message : 'Xato');
        }
    });
